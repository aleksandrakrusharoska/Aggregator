"""
Run the price anomaly detection agent against all ads in Supabase.

Usage:
    python run_price_anomaly_agent.py
    python run_price_anomaly_agent.py --source pazar3
"""
import argparse
import logging
import os
...
def fetch_ads(sb, source=None) -> list[dict]:
    ads, offset = [], 0
    while True:
        q = sb.table('ads').select('ad_url, title, price_eur, source')
        if source:
...
def main():
    ...
    log.info('Storing %d results...', len(results))
